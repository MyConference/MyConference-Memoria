...
Para configurar qué base de datos utilizar, se deben establecer la variable \texttt{MONGO\_URI} a una URI con esquema (protocolo) de tipo \texttt{mongodb:}, como por ejemplo \linebreak\texttt{mongodb://root:root@example.org/myconference}.
...
\begin{description}
  ...
  Debemos añadir a la aplicación una instancia de Transloadit. Al hacerlo se añadirán a la aplicación las variables \texttt{TRANSLOADIT\_URL}, \texttt{TRANSLOADIT\_AUTH\_KEY} y \texttt{TRANSLOADIT\_SECRET\_KEY}.
...
Para ello utilizaremos la herramienta de línea de comandos de Heroku, descrita en el punto \ref{sec:deploy-api-heroku}.
...
Debemos haber configurado el código para utilizar \texttt{git}, y haber apuntado el \textit{remote} \texttt{heroku} a la URL del repositorio de nuestra aplicación en Heroku (por ejemplo, \linebreak\texttt{git@heroku.com:myconference-web.git}).
