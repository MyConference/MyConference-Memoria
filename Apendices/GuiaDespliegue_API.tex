...
\begin{itemize}
  \item \texttt{HOST}: El nombre de dominio (host) completo de la API, como por ejemplo \linebreak\texttt{api.myconference.com}. Nótese que no deben aparecer separadores de ruta (\texttt{/}) ni prefijos de esquema (\texttt{http://}).
...
Debemos haber configurado el código para utilizar \texttt{git}, y haber apuntado el \textit{remote} \texttt{heroku} a la URL del repositorio de nuestra aplicación en Heroku (por ejemplo, \linebreak\texttt{git@heroku.com:myconference-api.git}).
