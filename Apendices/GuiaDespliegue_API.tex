...
\begin{lstlisting}[language=bash]
  $ heroku config:set NODE_ENV=production HOST=api.myconference.com WEB_URL=http://myconference.com
\end{lstlisting}
...
Debemos haber configurado el código para utilizar \texttt{git}, y haber apuntado el \textit{remote} \texttt{heroku} a la URL de nuestra aplicación en Heroku (\texttt{git@heroku.com:myconf-api-dev.git}).
