...
\begin{itemize}
	\item Crear una clase abstracta \texttt{MyConferenceActivity} que extendía a \texttt{SherlockFragmentActivity}.
	\item Crear una clase \texttt{MyConferenceFragment} que extendía a \texttt{SherlockFragment}.
\end{itemize}

A partir de ese momento, cualquier actividad que se creara extendía de \texttt{MyConferenceActivity} y cualquier fragmento de \texttt{MyConferenceFragment}. Esto hacía que automáticamente se introdujese la \textit{Action Bar} en la interfaz gráfica.

Para acceder a la \textit{Action Bar} solo era necesario llamar al método \texttt{getSupportActionBar()}, como si se utilizara la que viene con la librería de soporte, ya que \texttt{ActionBarSherlock} sobreescribe ese método y nos devuelve la suya. El resto de código utilizado ha sido habilitar el botón de "home", situado en la esquina izquierda de la barra, utilizando la función \texttt{setHomeButtonEnabled(true)}; y llamar a la función \texttt{setDisplayHomeAsUpEnabled(true)} para poder mostrar el icono de abertura y cierre del menú lateral. También hemos utilizado la función \texttt{setTitle(String title)} para poder cambiar el título al de la sección actual en la que nos encontráramos.


\figura{Vectorial/Android/actionBarSherlock}{width=0.8\textwidth}{fig:actionBarSherlock}%
    {\textit{Action Bar} en nuestra app gracias a \texttt{ActionBarSherlock}.}
...
\figura{Vectorial/Android/amazingListView}{width=0.5\textwidth,height=11cm}{fig:amazingListView}%
    {Captura del programa de un congreso utilizando \texttt{AmazingListView}.}
...
Lo primero fue añadir un marcador por cada lugar existente en la base de datos. Cuando un usuario pincha sobre el marcador, aparece una ventana de información con el nombre del lugar y si se pincha en esa ventana, lanzamos un \texttt{Intent} para abrir la aplicación de Google Maps instalada en el dispositivo con ese lugar pinchado como origen. Si el usuario no tuviese instalada la aplicación simplemente se muestra un mensaje de error en forma de \texttt{Toast} comentando que no ha sido posible abrir el mapa.
...
\figura{Vectorial/Android/googleMaps}{width=0.5\textwidth,height=11cm}{fig:googleMaps}%
    {Captura de la sección de lugares utilizando \textit{Google Maps}.}
...
También contiene observadores para que cuando se produzca algún cambio en la base de datos referente al ingreso o borrado de un congreso, se actualice el menú.
...
\figura{Vectorial/Android/accountManager}{width=\textwidth}{fig:accountManager}%
    {Diagrama UML de las clases del paquete es.ucm.myconference.accountmanager.}
...
\subsection{Campos comunes de las tablas}

Todas las tablas tienen una primera columna \texttt{ID} de tipo entero y autoincremental. Esto quiere decir que cada vez que se inserta un registro se le asigna un id diferente.

Las tablas también comparten una segunda columna denominada \texttt{conf\_uuid}, que es el identificador propio de cada congreso, ya que dentro de la app puede haber varios congresos a los que el usuario vaya a asistir y, por ejemplo, todos los anuncios se guardan en la misma tabla, y tiene que poderse diferenciar a qué congreso pertenece cada anuncio. Este identificador se consigue a partir de la tabla de \textit{Congresos}, cuyos campos se explican en la siguiente sección.

\subsection{Campos propios de cada tabla}

\paragraph{Congresos}

Se compone de 3 campos: el nombre del congreso, una descripción que luego se mostrará en la sección \textit{About} de la app, y el campo uuid mencionado anteriormente. Los 3 campos son de tipo \texttt{TEXT}.
