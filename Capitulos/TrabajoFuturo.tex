...
En este capítulo se explican las posibles ampliaciones que se podrían realizar.
...
%Explicación de lo que hay hecho DONE
%Explicación de lo que falta por hacer DONE
%Capturas de la aplicación
%Tecnologías necesarias (Xcode)
%Requisitos
...
\begin{itemize}
	\item Mostrar la lista de congresos del usuario en cuanto este inicie sesión en la aplicación.
	\item Cuando el usuario pulse sobre un congreso se mostraría un vista con la información resumida acerca del congreso que acaba de seleccionar y un menú que permita poder navegar entre las distintas secciones del congreso. La idea es que el menú sea el nativo que proporciona el SDK de iOS, que es una barra que aparece en la parte inferior de la pantalla con distintos items de navegación.
	\item Implementar cada una de las secciones del congreso, es decir, la sección de usuarios invitados, los anuncios del congreso, los documentos útiles, los lugares de interés, los organizadores y ponentes del congreso y la agenda.
	\item Implementar el sistema de cierre de sesión para informar a la API de que el usuario no va a seguir usando la sesión actual.
	\item Internacionalizar la aplicación para que pueda soportar varios idiomas. Actualmente la aplicación solo está disponible en inglés.
	\item Diseñar una base de datos interna que almacene los datos que se recogen de la API para mejorar el rendimiento de la aplicación.
\end{itemize}

Las figuras \ref{fig:ios-main}, \ref{fig:ios-signup} y \ref{fig:ios-login} muestran capturas de la aplicación iOS.

\figura{Bitmap/iOS/ios-main}{width=0.5\textwidth}{fig:ios-main}%
    {Pantalla principal de la aplicación iOS.}

\figura{Bitmap/iOS/ios-signup}{width=0.5\textwidth}{fig:ios-signup}%
    {Pantalla de registro de la aplicación iOS.}

\figura{Bitmap/iOS/ios-login}{width=0.5\textwidth}{fig:ios-login}%
...
\figura{Bitmap/iOS/xcode-logo}{width=0.5\textwidth}{fig:xcode-logo}%
    {Logo de Xcode.}
