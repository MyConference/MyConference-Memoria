...
A continuación se describen todos los servicios de la API disponibles, conel siguiente formato:
...
\paragraph{\texttt{POST /auth/signup}} ~\\

\begin{itemize}
  \item \texttt{application_id}: ID de aplicación desde la que se realiza la petición.
  \item \texttt{device_id}: ID del dispositivo desde el que se realiza la petición. 
  \item \texttt{user_data}: Un objeto JSON con los campos:
  \begin{itemize}
